...
namespace {
  ...
  void add_dominating_defs(Instruction* term, DenseSet<Value*>& invalid, DominatorTree* DT) {
...
static void ComputeInvalidSetAtStatepoint(CallSite& CS, DenseSet<Value*>& invalid,
                                          DominatorTree& DT) {
  Instruction* inst = CS.getInstruction();
  const int num_call_args = cast<ConstantInt>(CS.getArgument(1))->getZExtValue();
  const int num_stacks = cast<ConstantInt>(CS.getArgument(5))->getZExtValue();
  const int num_locals = cast<ConstantInt>(CS.getArgument(6))->getZExtValue();
  const int num_mon = cast<ConstantInt>(CS.getArgument(7))->getZExtValue();
  
  const int gc_begin = 8 + num_call_args + 2 * num_stacks + 2 * num_locals + num_mon;
  ...
  while( !worklist.empty() ) {
    ...
    for(BasicBlock::iterator itr = current->begin(), end = current->getFirstNonPHI();
      ...
      for(size_t i = 0; i < phi->getNumIncomingValues(); i++) {
        ...
        if( state[inBB].InvalidOnExit.find(InVal) != state[inBB].InvalidOnExit.end() ) {
          ...
          if (RelocationPHIEscapes(phi)) {
            assert( state[inBB].InvalidOnExit.find(InVal) == state[inBB].InvalidOnExit.end() &&
    ...
    if( set_equals(state[current].InvalidOnEntry, invalid) ) {
      continue;
    }
    ...
    for(BasicBlock::iterator itr = current->getFirstNonPHI(), end = current->end();
      ...
      if( isa<InvokeInst>(inst) || isa<CallInst>(inst) ) {
        ...
        if( F && F->getIntrinsicID() == Intrinsic::statepoint ) {
          ...
          assert( StatepointInvalidations.count(inst) &&
